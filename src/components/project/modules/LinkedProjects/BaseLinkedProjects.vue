...
import { addLinkedProject, deleteLinkedProject } from 'shared-projects-frontend/apis'
...
const onSubmit = (linkedProjects: TranslatedProject[]) => {
  ...
  addLinkedProject(props.project.id, body)
    .then((linkedProjects) => {
      linkedProjects.map((linked) => analytics.linkedProject.addLinkedProject(linked))
