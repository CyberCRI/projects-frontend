...
export default {
  ...
  methods: {
    async getAsPDF() {
              ...
              if (srcMatch && srcMatch[1]) {
                ...
                if (!src.startsWith('data:')) {
                  try {
                    ...
                    resolve([tag, '']) // Fallback to original src on error
                  }
                }
              }
            })
      ...
      const fixedDescription = await convertImages(this.project.description)

      const pdfContent = `<html><body><h1>${this.project.title}</h1><p>${fixedDescription}</p></body></html>`
