<template>
  <div v-if="!isLoading" class="mentroship-respond-page page-section-narrow page-top">
    <h1 class="page-title">
      {{ $t(isOffer ? 'mentorship.respond.title-mentoree' : 'mentorship.respond.title-mentor') }}
    </h1>
    ...
    <template v-else>
      ...
      <div class="form-field">
        <h3 class="label">
          {{ $t('mentorship.respond.add-comment') }}
        </h3>
        <p class="notice">
          {{
            $t(
  ...
  computed: {
    ...
    mentoreeName() {
      if (this.mentorship?.mentoree) {
        return this.mentorship.mentoree.given_name + ' ' + this.mentorship.mentoree.family_name
      }
      return ''
    },
