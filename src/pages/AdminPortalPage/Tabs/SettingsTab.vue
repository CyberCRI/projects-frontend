      ...
      <div class="block-container">
        ...
        >
          ...
          <div v-if="form.is_logo_visible_on_parent_dashboard" class="color-ctn">
            <span class="description">{{ $filters.capitalize($t('tips.organization-logo')) }}</span>
...
export default {
  ...
  validations() {
    return {
      form: {
        name: {
          required: helpers.withMessage(this.$t('admin.form.admin-info.name.required'), required),
        ...
        contact_email: {
          ...
          email: helpers.withMessage(this.$t('admin.form.admin-info.contact-email.email'), email),
        },
        language: {
          required: helpers.withMessage(
  ...
  methods: {
    async saveData() {
      ...
      try {
        ...
        this.toaster.pushError(`${this.$t('toasts.organization-general-update.error')} (${error})`)
        console.error(error)
      } finally {
