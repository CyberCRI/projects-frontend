...
import type { /*EventModel,*/ EventInput /*, EventOutput*/, EventModel } from '@/models/event.model'
...
export async function getEvent(orgCode: string, idOrSlug: number | string) {
  return await useAPI<EventModel>(`organization/${orgCode}/event/${idOrSlug}/`, {}) //.data.value
}
