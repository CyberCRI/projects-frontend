...
export async function offerMentorship(orgCode, skill, payload) {
    return (
        await useAPI(`organization/${orgCode}/mentoring/contact-mentoree/${skill.id}/`, {
            body: payload,
            method: 'POST',
        })
    ).data
...
export async function askMentorship(orgCode, skill, payload) {
    return (
        await useAPI(`organization/${orgCode}/mentoring/contact-mentor/${skill.id}/`, {
            body: payload,
            method: 'POST',
        })
    ).data
...
export async function respondMentorship(orgCode, mentorshipId, payload) {
    return (
        await useAPI(`organization/${orgCode}/mentoring/${mentorshipId}/respond/`, {
            body: payload,
            method: 'POST',
        })
    ).data
