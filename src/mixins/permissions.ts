...
export default defineComponent({
  computed: {
    ...
    isOrgAdmin() {
      ...
      return (
        this.isConnectedForPermissions &&
        this.getUserRolesForPermissions.some((role) => role === `organization:#${orgId}:admins`)
      )
    },

    isAdmin() {
      return (
        (this.isConnectedForPermissions && this.hasPermission('organizations', 'destroy')) ||
        this.isSuperAdmin ||
        this.isOrgAdmin
      )
    },
    ...
    isOrgUser() {
      ...
      return (
        this.isConnectedForPermissions &&
        this.getUserRolesForPermissions.some((role) => role === `organization:#${orgId}:users`)
      )
    },
    ...
    canAddReview() {
      return (
        this.hasPermission('projects', 'add_review', this.currentProjectIdForPermissions || null) ||
        this.hasPermission(
          'organization',
          'add_review',
          this.currentOrganizationForPermissions?.id
    ...
    canEditComment() {
      return (
        this.hasPermission('projects', 'change_comment', this.currentProjectIdForPermissions) ||
        this.hasPermission(
          'organizations',
          'change_comment',
          this.currentOrganizationForPermissions?.id
    ...
    canDeleteComment() {
      return (
        this.hasPermission('projects', 'delete_comment', this.currentProjectIdForPermissions) ||
        this.hasPermission(
          'organizations',
          'delete_comment',
          this.currentOrganizationForPermissions?.id
